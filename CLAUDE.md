...
| --- | --- | --- |
| Low | Text/docs/mechanical only; no behavior, public contract, science, security, dependency, or workflow change | Codex |
| Standard | Routine code, tests, UI, refactor, or bug fix not protecting a load-bearing area | Codex |
| Load-bearing | Science/clinical/reference data or their tests; privacy/security/auth; schema/migration/data loss; concurrency; dependencies; updater/installer/release; CI/workflows/permissions; core architecture; broad/hard-to-revert | Codex; CodeRabbit when a hosted second engine is wanted |

The lane column is capacity, not authority: the validator enforces only that the selected route sits at or above the changed-path floor, never which provider reviewed it. Nearly every path in this repository is load-bearing, so pinning the top route to one provider simply exhausts that provider (#2247). Codex is the default hosted lane on every route. Reach for CodeRabbit deliberately — apply the `review:coderabbit` label, which is the only thing that now starts a hosted CodeRabbit review. Copilot's envelope now matches what Copilot actually posts (#2248), so the lane is usable in principle; its binding constraint is account quota, since one Copilot review costs 13 premium requests and a refusal rather than a review is the usual outcome once that runs out. Check a recent PR for a quota refusal before selecting it.
