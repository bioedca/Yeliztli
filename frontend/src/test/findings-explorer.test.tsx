...
describe("FindingsExplorer", () => {
  ...
  function setupMinStarsAwareFetchMock() {
    mockFetch.mockImplementation(async (url: string) => {
      ...
      return {
        ok: true,
        json: async () =>
          TIERED_FINDINGS.filter((finding) => (finding.evidence_level ?? 0) >= minStars),
      }
    })
  }
  ...
  function SameRouteNavigation() {
    ...
    return (
      <>
        ...
        <Link to="/findings?sample_id=1&minStars=3">High-confidence findings</Link>
  ...
  it("applies minStars when it appears on the mounted route and drops it again on history back (#2047)", async () => {
    ...
    fireEvent.click(screen.getByRole("link", { name: "High-confidence findings" }))
