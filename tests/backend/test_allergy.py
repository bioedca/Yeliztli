...
ALL_ALLERGY_VARIANTS = [
    ...
    ("rs7454108", "6", 32681483, "TT"),  # HLA-DQ8 proxy ref (no proxy; plus C=risk, #875)
...
class TestCeliacCombined:
    def test_neither_dq2_nor_dq8(
        ...
        _seed_variants(
            ...
            [
                ...
                ("rs7454108", "6", 32681483, "TT"),  # DQ8 ref (no proxy; plus C=risk, #875)
            ],
        )
    ...
    def test_dq8_only(
        ...
        _seed_variants(
            ...
            [
                ("rs2187668", "6", 32605884, "CC"),  # DQ2 ref
                ("rs7454108", "6", 32681483, "CT"),  # DQ8 het
            ],
        )
        _seed_hla_proxies(reference_engine)
        result = score_allergy_pathways(panel, sample_engine, reference_engine)
    ...
    def test_neither_proxy_genotyped_is_indeterminate(
        ...
        A consumer array that types neither rs2187668 nor rs7454108 has no celiac
    ...
    def test_one_proxy_missing_is_indeterminate(
        ...
        # DQ8 ref only (TT = no proxy; plus-strand C=risk, #875)
        _seed_variants(sample_engine, [("rs7454108", "6", 32681483, "TT")])
        _seed_hla_proxies(reference_engine)
        result = score_allergy_pathways(panel, sample_engine, reference_engine)
        assert result.celiac_combined is not None
        assert result.celiac_combined.state == "indeterminate"
    ...
    def test_no_call_proxy_is_indeterminate(
        ...
        _seed_variants(
            ...
            [
                ("rs2187668", "6", 32605884, "CC"),  # DQ2 ref
                ("rs7454108", "6", 32681483, "--"),  # DQ8 no-call
            ],
        )
        _seed_hla_proxies(reference_engine)
        result = score_allergy_pathways(panel, sample_engine, reference_engine)
...
# ── Celiac HLA-DQ8 proxy marker identity regression (#875) ──────────────────


class TestCeliacDQ8ProxyMarkerIdentity:
    """#875: DQ8 uses rs7454108, not rs7775228.

    Monsuur 2008 (PMID 18509540) reports rs7454108 as the DQ8 tag SNP. The
    study reports the tagging allele on the opposite strand; Ensembl GRCh37
    reports plus-strand T/C at 6:32681483, so this panel's plus-strand risk
    allele is C. rs7775228 belongs to the DQ2.2/DQ4 haplotype marker set and
    must not drive DQ8 calls.
    """

    def _snp(self, panel: AllergyPanel) -> PanelSNP:
        return next(s for pw in panel.pathways for s in pw.snps if s.rsid == "rs7454108")

    def test_rs7775228_is_not_used_as_dq8_proxy(self, panel: AllergyPanel) -> None:
        assert all(s.rsid != "rs7775228" for pw in panel.pathways for s in pw.snps)

    def test_minor_allele_c_is_the_risk_allele(self, panel: AllergyPanel) -> None:
        snp = self._snp(panel)
        # Plus-strand C = HLA-DQ8 proxy tag (risk); plus-strand T = ref.
    ...
    def test_combined_dq8_positivity_tracks_the_minor_allele(
        ...
        """The combined DQ2/DQ8 assessment must flag the plus-strand C carrier
        as DQ8-positive and the common TT genotype as DQ8-negative. (DQ2 held
        at its non-risk TT-... i.e. CC ref.)"""
        # Common TT majority: DQ8-negative; DQ2 ref (CC) too → 'neither'.
        _seed_variants(
            sample_engine,
            [
                ("rs2187668", "6", 32605884, "CC"),  # DQ2 ref
                ("rs7454108", "6", 32681483, "TT"),  # DQ8 ref (no proxy)
            ],
        )
        _seed_hla_proxies(reference_engine)
    ...
    def test_combined_plus_strand_c_homozygote_is_dq8_positive(
        self,
        panel: AllergyPanel,
        sample_engine: sa.Engine,
        reference_engine: sa.Engine,
    ) -> None:
        """A CC DQ8-tag homozygote (DQ2 ref) maps to 'dq8_only', not rule-out."""
        _seed_variants(
            sample_engine,
            [
                ("rs2187668", "6", 32605884, "CC"),  # DQ2 ref
                ("rs7454108", "6", 32681483, "CC"),  # DQ8 homozygous proxy (plus C)
            ],
        )
        _seed_hla_proxies(reference_engine)
        result = score_allergy_pathways(panel, sample_engine, reference_engine)
...
class TestFindingsStorage:
    ...
    def test_dq8_only_pathway_detail_does_not_rule_out_from_dq2_negative(
        ...
        celiac_details = {
            ...
            if detail["rsid"] in {"rs2187668", "rs7454108"}
        }
        assert set(celiac_details) == {"rs2187668", "rs7454108"}
