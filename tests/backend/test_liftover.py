...
class TestBatchLiftoverRoute:
    ...
    def test_deleting_the_sample_mid_batch_is_refused(
        self, liftover_sample_client: TestClient, tmp_data_dir: Path, monkeypatch
    ) -> None:
        """Deleting a sample while its batch converts must not strand its file.

        The batch holds an open engine for the whole read → convert → write
        span. A delete that lands in the middle unlinks the database and then
        the batch reconnects through that retained engine, which recreates an
        empty ``sample_1.db`` *after* the DELETE reported success. Sample IDs
        and their file names are reused, so the next sample to take id 1
        inherits the leftover file.

        The interleaving is driven from inside ``batch_convert``: the DELETE is
        issued at the one moment the lease is held and the write has not
        happened yet, which is the exact window the bug lives in.
        """
        sample_db = tmp_data_dir / "samples" / "sample_1.db"
        real_batch_convert = liftover_module.batch_convert
        observed: dict[str, int] = {}

        def delete_midway(variants):
            observed["delete_status"] = liftover_sample_client.delete("/api/samples/1").status_code
            return real_batch_convert(variants)

        monkeypatch.setattr(
            "backend.api.routes.liftover.batch_convert", delete_midway, raising=True
        )

        try:
            post_status: object = liftover_sample_client.post("/api/liftover/1").status_code
        except Exception as exc:  # noqa: BLE001
            # Unguarded, the delete disposes the engine and unlinks the file,
            # then the batch's write reconnects — recreating an empty database
            # and raising ``no such table: annotated_variants`` *after* the
            # DELETE already answered 204. Captured rather than propagated so
            # the assertions below report the state left on disk, which is the
            # part that outlives the request.
            post_status = f"raised {type(exc).__name__}"

        assert observed.get("delete_status") is not None, "the delete never ran"
        engine = sa.create_engine(f"sqlite:///{tmp_data_dir / 'reference.db'}")
        try:
            with engine.connect() as conn:
                row_exists = (
                    conn.execute(
                        sa.select(sa.func.count()).select_from(samples).where(samples.c.id == 1)
                    ).scalar()
                    or 0
                ) > 0
        finally:
        ...
        # The registry row and the file on disk must agree. They disagree in
        # exactly one way here: the DELETE succeeded, and the batch then put the
        # file back — a stale sample_1.db in a namespace that gets reused.
        assert row_exists == sample_db.exists(), (
            "registry row and database file disagree — a stale sample_1.db was "
            f"left behind (row_exists={row_exists}, file_exists={sample_db.exists()})"
        )
        assert observed["delete_status"] == 409, (
            "deleting a sample while its liftover batch holds the lease must be "
            f"refused, got {observed['delete_status']}"
        )
        assert row_exists, "the sample was deleted out from under the running batch"
        assert post_status == 200, post_status
        assert _grch38_by_rsid(tmp_data_dir)["rs1801133"] == ("1", 11796321)

    def test_delete_succeeds_once_the_batch_has_finished(
        self, liftover_sample_client: TestClient, tmp_data_dir: Path
    ) -> None:
        """The refusal is scoped to the lease, not a permanent block.

        Paired with the test above so the guard cannot be satisfied by simply
        refusing every deletion: once the batch releases its lease the sample
        deletes normally and takes its file with it.
        """
        assert liftover_sample_client.post("/api/liftover/1").status_code == 200

        sample_db = tmp_data_dir / "samples" / "sample_1.db"
        assert sample_db.exists(), "precondition: the sample database is on disk"

        assert liftover_sample_client.delete("/api/samples/1").status_code == 204
        assert not sample_db.exists(), "the deleted sample left its database behind"
