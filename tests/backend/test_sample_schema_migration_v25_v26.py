"""Tests for the v25 -> v26 persisted rsID-doubling repair (#2051)."""
...
def test_v26_repairs_the_doubled_rsid_in_every_affected_module(
...
def test_v26_leaves_rows_without_the_fingerprint_untouched(sample_engine: sa.Engine) -> None:
    rows = [
...
def test_v26_repairs_finding_diff_entries_without_changing_counts(
    sample_engine: sa.Engine,
) -> None:
...
def test_v26_repairs_findings_without_optional_columns() -> None:
...
def test_v26_locks_before_reading_or_writing_findings_and_diff(
    sample_engine: sa.Engine,
) -> None:
    diff = {
...
def test_v26_rolls_back_all_repairs_when_one_update_fails(sample_engine: sa.Engine) -> None:
