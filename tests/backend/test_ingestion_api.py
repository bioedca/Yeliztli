...
from backend.ingestion.base import ParserError
...
SMALL_23ANDME_V3_MINUS_STRAND_HAPLOID = (
    b"# This data file generated by 23andMe\n"
    b"# reference human assembly build 36\n"
    b"# rsid\tchromosome\tposition\tgenotype\n"
    b"i_issue1616_minus_haploid\t1\t2480001\tA\n"
)
...
def _ingest_core_with_tmp_registry(
    tmp_data_dir: Path,
    ...
    try:
        return ingest_route._ingest_file(file_bytes, filename)
    finally:
...
class TestIngestEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_ingest_concatenated_vendor_file_returns_actionable_422(
        ...
        concatenated = (
            ...
            b"# rsid\tchromosome\tposition\tgenotype\n"
            b"rs1\t1\t100\tAA\n"
            b"#This file was generated by AncestryDNA at: Tue Jan 14 2025\n"
            b"#Data was collected using AncestryDNA array version: V2.0\n"
        ...
        class _Registry:
            def __init__(self) -> None:
                self.settings = Settings(data_dir=tmp_data_dir, wal_mode=False)
        ...
        with pytest.raises(HTTPException) as excinfo:
            await ingest_route.ingest_file(_Upload())

        exc = excinfo.value
        assert exc.status_code == 422
        detail = exc.detail
        assert "multiple concatenated raw data files" in detail
        assert "single-vendor" in detail
        assert "expected 4 columns" not in detail

    def test_ingest_single_member_vendor_zip_persists_like_inner_txt(
        self, tmp_data_dir, monkeypatch
    ):
        raw = SMALL_23ANDME_V4
        archive = _zip_bytes({"sample_23andme_v4.txt": raw})

        data = _ingest_core_with_tmp_registry(
            tmp_data_dir,
            monkeypatch,
            archive,
            "sample_23andme_v4.zip",
        )

        assert data["file_format"] == "23andme_v4"
        ...
        engine.dispose()
        assert row.name == "sample_23andme_v4.txt"
        assert row.file_hash == hashlib.sha256(raw).hexdigest()

    def test_ingest_multi_member_zip_rejected(self):
        from backend.api.routes import ingest as ingest_route

        raw = SMALL_23ANDME_V4
        archive = _zip_bytes({"one.txt": raw, "two.txt": raw})

        with pytest.raises(ParserError) as excinfo:
            ingest_route._ingest_file(archive, "genome.zip")
        assert "exactly one raw 23andMe/AncestryDNA .txt file" in str(excinfo.value)

    def test_ingest_oversized_zip_member_rejected(self, monkeypatch):
        from backend.api.routes import ingest as ingest_route

        monkeypatch.setattr(ingest_route, "_MAX_RAW_UPLOAD_BYTES", 5)
        archive = _zip_bytes({"sample.txt": b"123456"})

        with pytest.raises(ParserError) as excinfo:
            ingest_route._ingest_file(archive, "genome.zip")
        assert "exceeds the 5 bytes upload limit" in str(excinfo.value)

    def test_ingest_zip_without_txt_member_rejected(self):
        from backend.api.routes import ingest as ingest_route

        archive = _zip_bytes({"sample.csv": SMALL_23ANDME_V4})

        with pytest.raises(ParserError) as excinfo:
            ingest_route._ingest_file(archive, "genome.zip")
        assert "does not contain a raw 23andMe/AncestryDNA .txt file" in str(excinfo.value)

    def test_ingest_non_genotype_zip_txt_rejected(self, tmp_data_dir, monkeypatch):
        archive = _zip_bytes({"notes.txt": b"hello world\nnot genotype data\n"})

        with pytest.raises(ParserError) as excinfo:
            _ingest_core_with_tmp_registry(tmp_data_dir, monkeypatch, archive, "notes.zip")
        assert "Unrecognized file format" in str(excinfo.value)

    def test_ingest_malformed_zip_rejected_before_registry_setup(self, monkeypatch):
        from backend.api.routes import ingest as ingest_route

        def fail_get_registry():
            raise AssertionError("Malformed ZIP rejection should happen before registry setup")
...
class TestGenomeBuildGate:
    ...
    @pytest.mark.asyncio
    async def test_v3_minus_strand_haploid_call_stored_complemented(
        self, tmp_data_dir, monkeypatch
    ):
        """A single-character call on a strand-flipped build-36 segment is stored
        as the complemented GRCh37 plus-strand allele, not the raw uploaded base."""
        from backend.api.routes import ingest as ingest_route
        from backend.db.connection import DBRegistry, reset_registry
        ...
        class _Upload:
            filename = "minus_strand_haploid_v3.txt"

            async def read(self) -> bytes:
                return SMALL_23ANDME_V3_MINUS_STRAND_HAPLOID

        try:
            response = await ingest_route.ingest_file(_Upload())
        finally:
            registry.dispose_all()
            reset_registry()

        assert response["file_format"] == "23andme_v3"
        ...
        with engine.connect() as conn:
            row = conn.execute(
                sa.select(raw_variants.c.chrom, raw_variants.c.pos, raw_variants.c.genotype).where(
                    raw_variants.c.rsid == "i_issue1616_minus_haploid"
                )
            ).fetchone()
        engine.dispose()
        assert row is not None, "minus-strand haploid call should have lifted and stored"
        assert (row.chrom, row.pos, row.genotype) == ("1", 2494417, "T")
