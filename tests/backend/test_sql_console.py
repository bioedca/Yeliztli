...
class TestSqlConsoleReadAccess:
    ...
    def test_non_audit_authorization_error_is_not_audit_only(self, client) -> None:
        """#2019: bare SQLite authorization errors need a guard-trigger signal."""
        tc, sid = client
        resp = tc.post(
            "/api/query/sql",
            json={"sample_id": sid, "sql": "SELECT load_extension('missing')"},
        )
