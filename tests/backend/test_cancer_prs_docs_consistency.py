...
from backend.analysis.cancer_prs import load_cancer_prs_weights
...
# A numbered entry ends at the next numbered entry *or* at a footnote definition.
# Without the `\^` alternative the last entry absorbs any `[^label]:` block that
# follows it -- and cancer.md carries one -- so its identifiers would be read as
# though this note had cited them.
