...
Ten ``store_*_findings()`` functions previously skipped the ``sa.delete(findings)``
that clears the module's prior rows on an empty rerun — nine via ``if not rows:
return 0`` *before* the delete, and ``store_traits_findings`` via gating its non-PRS
delete behind ``if rows:`` (#1240). So an empty rerun (a replaced sample that yields
no current reportable findings) left the previous run's findings in place — a stale,
user-facing data-integrity bug, the same class as cancer #252 and the PRS stale paths
(#149/#150/#244/#245).
...
from backend.analysis.traits import TraitsResult, store_traits_findings
...
_CASES = [
    ...
    # traits gated its non-PRS clear behind `if rows:` rather than an early return, so an
    # empty rerun skipped the delete and left stale pathway/SNP/cross-module rows (#1240).
    ("traits", "traits", store_traits_findings, TraitsResult),
]
