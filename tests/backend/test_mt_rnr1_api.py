"""Tests for the MT-RNR1 aminoglycoside-ototoxicity API (factory-built router)."""
...
def _env(tmp_path: Path) -> Generator[sa.Engine, None, None]:
    ...
    with sample_engine.begin() as conn:
        conn.execute(
            ...
            [{"rsid": "rs267606617", "chrom": "MT", "pos": 1555, "genotype": "G"}],
        )
...
def client(_env: sa.Engine) -> TestClient:
    from backend.api.routes.mt_rnr1 import router
...
class TestDisclaimer:
    def test_returns_disclaimer(self, client: TestClient) -> None:
        resp = client.get("/api/analysis/mt-rnr1/disclaimer")
        assert resp.status_code == 200
        data = resp.json()
        assert data["title"] == MT_RNR1_DISCLAIMER_TITLE
        assert data["text"] == MT_RNR1_DISCLAIMER_TEXT
        assert "decision-support, not a prescription" in data["text"].lower()


class TestRunAndList:
    def test_run_then_list(self, client: TestClient) -> None:
        run = client.post("/api/analysis/mt-rnr1/run?sample_id=1")
        assert run.status_code == 200
        assert run.json()["findings_count"] == 1

        listing = client.get("/api/analysis/mt-rnr1/findings?sample_id=1")
        assert listing.status_code == 200
        item = listing.json()["items"][0]
        assert item["gene_symbol"] == "MT-RNR1"
        assert item["evidence_level"] == 3
        assert "aminoglycoside" in item["finding_text"].lower()

    def test_run_idempotent(self, client: TestClient) -> None:
        client.post("/api/analysis/mt-rnr1/run?sample_id=1")
        client.post("/api/analysis/mt-rnr1/run?sample_id=1")
        listing = client.get("/api/analysis/mt-rnr1/findings?sample_id=1")
