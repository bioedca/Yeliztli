...
class TestStoredFindingTextGeneLabel:
    ...
    this drives ``store_traits_findings`` itself, so a call site that rebuilds the
    ...
    def test_gene_prefixed_loci_persist_without_doubling(
        ...
        Traits has a single per-SNP text branch; the gene-prefixed loci are
        CRHR1, KATNAL2, CADM2 and the DRD4 exon III VNTR proxy (which also
        carries a coverage note and a cross-module link, neither of which
        touches the card's opening label).
        """
        loci = gene_prefixed_loci(panel)
        assert loci, "traits panel has no gene-prefixed locus; this guard would be vacuous"
        ...
        assert doubled == [], f"stored traits text prints a gene twice: {doubled}"
