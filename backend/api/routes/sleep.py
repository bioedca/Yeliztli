...
POST /api/analysis/sleep/run?sample_id=N                — Run/re-run scoring
"""
...
class SNPDetail(BaseModel):
    ...
    pmids: list[str] = []
    metabolizer_state: str | None = None
...
class MetabolizerState(BaseModel):
...
class PathwaysResponse(BaseModel):
    ...
    metabolizer: MetabolizerState | None = None
...
class RunResponse(BaseModel):
    """Result of running sleep scoring."""
...
def _fetch_sleep_findings(
    sample_engine: sa.Engine,
) -> list[dict[str, Any]]:
    """Fetch all sleep findings from the sample DB."""
    with sample_engine.connect() as conn:
        stmt = sa.select(findings).where(findings.c.module == "sleep").order_by(findings.c.id)
...
def list_pathways(
    ...
    """List all sleep pathway results for a sample.

    Returns each of the four sleep pathways with their categorical
    level (Elevated / Moderate / Standard), evidence level, and SNP counts.

    Example: ``GET /api/analysis/sleep/pathways?sample_id=1``
    """
    sample_engine = _get_sample_engine(sample_id)
    all_findings = _fetch_sleep_findings(sample_engine)
    ...
    # Metabolizer state
    metabolizer_findings = [f for f in all_findings if f["category"] == "metabolizer_state"]
    metabolizer = None
    if metabolizer_findings:
        met_detail = metabolizer_findings[0]["detail"]
        metabolizer = MetabolizerState(
            state=met_detail.get("metabolizer_state"),
        )

    return PathwaysResponse(
        items=items, total=len(items), cross_module=cross_items, metabolizer=metabolizer
    )
...
def pathway_detail(
    ...
    """Get detailed results for a single sleep pathway.

    Returns the pathway-level summary plus per-SNP breakdown with
    genotype, category, effect summary, CYP1A2 metabolizer state,
    and coverage notes.

    Example: ``GET /api/analysis/sleep/pathway/caffeine_sleep?sample_id=1``
    """
    sample_engine = _get_sample_engine(sample_id)
    all_findings = _fetch_sleep_findings(sample_engine)
    ...
    for sd in snp_details_data:
        ...
        snp_details.append(
            SNPDetail(
                ...
                pmids=pmids,
                metabolizer_state=sd.get("metabolizer_state"),
...
def run_sleep(
    ...
    logger.info("Sleep GWAS annotation_coverage updated for %d variants", gwas_updated)
