...
from backend.ingestion.parser_ancestrydna import ANCESTRYDNA_HEADER_TOKENS
...
_ERR_CONCATENATED_ANCESTRYDNA = (
    "this file appears to contain multiple concatenated raw data files "
    "(a 23andMe section followed by an AncestryDNA section). Please upload "
    "a single-vendor 23andMe or AncestryDNA raw-data .txt file."
)
...
def _is_ancestrydna_section_marker(line: str) -> bool:
    stripped = line.strip().lower()
    return any(marker in stripped for marker in _ANCESTRYDNA_SECTION_MARKERS)


def _is_ancestrydna_column_header(parts: list[str]) -> bool:
    columns = tuple(part.strip().lower() for part in parts)
    return columns == ANCESTRYDNA_HEADER_TOKENS


def _concatenated_ancestrydna_error(line_num: int) -> MalformedDataError:
    return MalformedDataError(f"Line {line_num}: {_ERR_CONCATENATED_ANCESTRYDNA}")
...
def _validate_line(parts: list[str], line_num: int) -> ParsedVariant:
    ...
    if _is_ancestrydna_column_header(parts):
        raise _concatenated_ancestrydna_error(line_num)
    ...
    try:
        ...
        for line_num, raw_line in enumerate(stream, start=1):
            ...
            if not line:
                ...
                continue
            if line.startswith("#"):
                if _is_ancestrydna_section_marker(line):
                    raise _concatenated_ancestrydna_error(line_num)
                skipped_lines += 1
                continue
