...
- Join the highest non-expired provisional generation eligible under the rules below. If none exists, use exactly `max_seen_generation + 1` only when no `active`, `waiting`, or `handoff` record is live; otherwise stop unless a named handoff or human reassignment explicitly authorizes the successor generation. Post `candidate`, wait five seconds, refetch every page twice, and elect lowest immutable comment ID in that generation; the winner marks `active` and refetches again. Stop before goal/worktree/code writes unless confirmed winner.
