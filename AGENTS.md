...
- Join the highest non-expired provisional generation eligible under the rules below. If none exists, use exactly `max_seen_generation + 1` only when no `active`, `waiting`, or `handoff` record is live; otherwise stop unless a named handoff or human reassignment explicitly authorizes the successor generation. The first eligible candidate's server `created_at + 5 seconds` fixes the election deadline; only candidates created by it compete. After it passes, refetch every page twice, require matching snapshots, and elect the lowest immutable comment ID. The winner marks `active`, refetches every page, and permits writes only if it is the sole live active claim with that ID and generation; multiple actives, disagreement, fetch failure, or mismatch stops all writes.
- `candidate`/`accept` expire after two minutes; `active`/`waiting`/`handoff` are live for 90 minutes from server `updated_at`. Heartbeat every 30 minutes and on state/SHA/PR changes. Before first edit, after resume, and before commit, push, PR mutation, Slurm submission, or merge, refetch and require the winning comment ID + generation, `state=active`, an unexpired `updated_at` lease, and that it remains the sole live active claim; any fetch, state, lease, ownership, or snapshot mismatch stops all writes.
