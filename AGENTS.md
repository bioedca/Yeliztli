...
- Join the highest non-expired provisional generation eligible under the rules below. If none exists, use exactly `max_seen_generation + 1` only when no `active`, `waiting`, or `handoff` record is live; otherwise stop unless a named handoff or human reassignment explicitly authorizes the successor generation. Every `candidate` or `accept` election uses the first eligible contender's server `created_at + 5 seconds` as its deadline; only contenders created by it compete. After it passes, refetch every page twice, require matching snapshots, and elect the lowest immutable comment ID. The winner marks `active`, refetches every page, and permits writes only if it is the sole live active claim with that ID/generation; multiple actives, disagreement, fetch failure, or mismatch stops all writes.
...
- Use `/review`, `codex review --uncommitted`, or `$coderabbit:code-review` iteratively. The final local gate is `codex review --base origin/main` (or an equivalent CodeRabbit review) on the committed head in its dedicated worktree. When Codex is selected, re-fetch the selected lane and head, then post exact `@codex review`; for a canonical clean comment, require GitHub's full-OID lookup to equal the freshly fetched PR head, the bot's 10-hex marker to equal that head's first 10 hex, and the head's canonical abbreviation to be no longer than 10 hex. Reject mismatch or ambiguity.
...
- After green, close/update the issue and Project; remove only the proven-merged branch/worktree, task artifacts, and task-owned terminal metadata that fresh ownership/state checks prove is unreferenced by any live generation. Preserve unrelated or uncertainly owned metadata.
