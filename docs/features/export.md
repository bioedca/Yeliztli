...
| **FHIR R4** | DiagnosticReport Bundle (JSON) in the FHIR R4 genomics-reporting format, for interoperability with research/genomics tooling — **not a clinical diagnostic report** (see below). Variant coordinates are exported in the GRCh37/hg19 reference frame. |
...
!!! warning "The FHIR export is not a clinical diagnostic report"
    ...
    LOINC-coded genomic reference-sequence component for the GRCh37/hg19 build, so
    chromosome positions are not exported as build-ambiguous values.
