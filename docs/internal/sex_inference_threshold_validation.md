...
| `THRESHOLD_X_HET_DIPLOID` | `0.15` | non-PAR chrX het **rate** at/above this is two X (XX / XXY) — issue #519 |
...
2. **X dosage by the non-PAR chrX heterozygosity *rate*, not a binary count (issue #519).**
   ...
   *candidate XY* (tolerates noise); a rate ≥ `THRESHOLD_X_HET_DIPLOID` (0.15) is
...
- **5,998** heterozygous non-PAR chrX calls — a female-level non-PAR chrX het
  *rate*, far above the `THRESHOLD_X_HET_DIPLOID` (0.15) diploid-X cutoff (#519).
